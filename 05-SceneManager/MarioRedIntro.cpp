...
CMarioRedIntro::CMarioRedIntro(float x, float y) : CGameObject(x, y) {
...
void CMarioRedIntro::Update(DWORD dt, vector<LPGAMEOBJECT>* coObjects)
{
...
void CMarioRedIntro::OnNoCollision(DWORD dt)
{
	x += vx * dt;
	y += vy * dt;
}

void CMarioRedIntro::OnCollisionWith(LPCOLLISIONEVENT e)
{
...
void CMarioRedIntro::OnCollisionWithButton(LPCOLLISIONEVENT e) {
	CButton* button = dynamic_cast<CButton*>(e->obj);
	button->SetIsCollected(true);
}
void CMarioRedIntro::OnCollisionWithBoomBrick(LPCOLLISIONEVENT e) {
...
void CMarioRedIntro::OnCollisionWithCard(LPCOLLISIONEVENT e) {
...
void CMarioRedIntro::OnCollisionWithPlatForm(LPCOLLISIONEVENT e) {
...
void CMarioRedIntro::OnCollisionWithPlantEnemy(LPCOLLISIONEVENT e) {
...
void CMarioRedIntro::OnCollisionWithFireFromPlant(LPCOLLISIONEVENT e) {
	if (untouchable) return;
...
void CMarioRedIntro::OnCollisionWithKoopa(LPCOLLISIONEVENT e) {
...
void CMarioRedIntro::OnCollisionWithGoomba(LPCOLLISIONEVENT e)
{
...
void CMarioRedIntro::OnCollisionWithCoin(LPCOLLISIONEVENT e)
{
...
void CMarioRedIntro::OnCollisionWithLeaf(LPCOLLISIONEVENT e) {
	e->obj->Delete();
	score += 1000;
	AddScore(x, y, 1000);
...
void CMarioRedIntro::OnCollisionWithMushRoom(LPCOLLISIONEVENT e)
{
...
void CMarioRedIntro::OnCollisionWithBrickQuestion(LPCOLLISIONEVENT e) {
...
void CMarioRedIntro::OnCollisionWithFlowerFire(LPCOLLISIONEVENT e) {
	if (!e->obj->IsDeleted())
	{
		score += 1000;
		AddScore(x, y, 1000);
...
void CMarioRedIntro::OnCollisionWithPortal(LPCOLLISIONEVENT e)
{
...
//
int CMarioRedIntro::GetAniIdTail()
{
	int aniId = -1;
	if (!isUsePipe) {
		if (!isFlying) {
...
int CMarioRedIntro::GetAniIdFire()
{
	int aniId = -1;
	if (!isShoot) {
		if (!isOnPlatform)
...
int CMarioRedIntro::GetAniIdSmall()
{
	int aniId = -1;
	if (!isChanging) {
		if (!isOnPlatform)
...
int CMarioRedIntro::GetAniIdBig()
{
	int aniId = -1;
	if (!isChanging) {
		if (!isOnPlatform)
...
void CMarioRedIntro::SetState(int state)
{
...
void CMarioRedIntro::GetBoundingBox(float& left, float& top, float& right, float& bottom)
{
...
void CMarioRedIntro::SetLevel(int l)
{
...
void CMarioRedIntro::BlockIfNoBlock(LPGAMEOBJECT gameobject) {
	if (level == MARIO_LEVEL_SMALL) {
...
void CMarioRedIntro::SetLevelLower() {
	isLower = true;
...
void CMarioRedIntro::SetMarioTailAttack() {
	if (level == MARIO_LEVEL_TAIL) {
		isTailAttack = true;
	}
}

void CMarioRedIntro::AddChangeAnimation() {
	CPlayScene* scene = (CPlayScene*)CGame::GetInstance()->GetCurrentScene();
	CEffect* effect = new CEffect(x, y, EFFECT_CHANGE);
	scene->AddObject(effect);
}

void CMarioRedIntro::AddEffectAttack(float xTemp, float yTemp) {
	CPlayScene* scene = (CPlayScene*)CGame::GetInstance()->GetCurrentScene();
	CEffect* effect = new CEffect(xTemp, yTemp, EFFECT_ATTACK);
	scene->AddObject(effect);
}
//Trong AddScore, thong nhat AddScore(x,y,0) la + 1 Up, con lai la + score
void CMarioRedIntro::AddScore(float xTemp, float yTemp, int scoreAdd) {
	CPlayScene* scene = (CPlayScene*)CGame::GetInstance()->GetCurrentScene();
	if (scoreAdd == 100) {
...
void CMarioRedIntro::IncreaseScoreUpCollision(float xTemp, float yTemp) {
...
bool CMarioRedIntro::MarioInDeadZone() { return y > POSITION_Y_DIE; }

void CMarioRedIntro::AdjustLogicSitting() {
	if (nx > 0) {
		if (vx > 0) {
...
void CMarioRedIntro::ChangeWorldMapWhenDie() {
	CData* data = CGame::GetInstance()->GetDataGame();
...
void CMarioRedIntro::ChangeWorldMapWhenNotDie() {
...
void CMarioRedIntro::SettingMarioAutoMoveEndPlayScene()
{
...
void CMarioRedIntro::AddEffectEndWorldFont1() {
	CPlayScene* scene = (CPlayScene*)CGame::GetInstance()->GetCurrentScene();
	if (!isEndScene) {
...
void CMarioRedIntro::AddEffectEndWorldFont2() {
	CPlayScene* scene = (CPlayScene*)CGame::GetInstance()->GetCurrentScene();
...
void CMarioRedIntro::AdjustHoldingKoopa() {
	if (GetTickCount64() - start_holding > TIME_MAX_HOLDING) {
		isHolding = false;
		start_holding = 0;
	}
}
void CMarioRedIntro::DownTimeClock1Second() {
	if (clock > 0) {
...
void CMarioRedIntro::DownTimeClockAndAddScore() {
...
void CMarioRedIntro::SaveDataGame() {
